class lpGeneratorService{
    ...
    buildOverlayByButtons(obj){
        jq_181(function($){
            ...
            for (var i = 0; i <obj.options.overlay.byButtons.elements.length; i ++ ){
                console.log(obj.options.overlay.byButtons.elements[i].id);
    ...
    buildOverlayByVkInvite (obj) {
        jq_181(function($){
            if (!VK) $('body').append('<script type="text/javascript" src="//vk.com/js/api/openapi.js?133"></script>');
            ...
            $('#vkInviteOverlay').overlay(obj.options.overlay.vkInviteOverlay.overlayOptions || { top: '10%', mask: { color:'#000', opacity:.5 }});
            $('#vkInviteOverlay').css(obj.options.overlay.vkInviteOverlay.boxCss || {'max-height':'220px' ,'width':'400px'});
        });
    };
}
